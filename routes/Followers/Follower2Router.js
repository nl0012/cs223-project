/**
* File name: Follower2Router
* Functionality: This file serves as the startpoint for the Follower 2 API Endpoint.
...
router.post('/', (req,res) => {
   ...
   if ('read_only' in req.body) {
    ...
    if(commit_set.length > 0) {
         ...
         followerModel.logTransaction(log,'follower2');
         res.status(200).end();
        })
    }

    if(read_only.length > 0){
        Promise.all(followerModel.readOnlyTransaction(read_only,'follower2',pool)).then((reads)=> {
